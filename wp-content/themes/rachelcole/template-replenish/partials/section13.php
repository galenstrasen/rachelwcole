<section class="section13 collage">
  <div class="container-fluid">
      <div class="row">
      <div class="col-sm-6 col-sm-push-6 col-xs-12">
          <h3>About Rachel</h3>
          <?php if(get_field('content13')) { the_field('content13'); } ?>
        </div><!-- /col -->
        <div class="col-sm-6 col-sm-pull-6 col-xs-12">
          <figure>
          <img src="<?php bloginfo('template_url')?>/template-replenish/assets/img/image-collage_5.jpg" class="img-responsive" alt="Rachel and her mom in Esperero Canyon in 1984" />
          <figcaption>
          That's me and my mom, in Esperoro Canyon in about 1984.<br/> It's always been my happy place.
          </figcaption>
          </figure>
        </div>

      </div>
    </div><!-- /container -->
</section>